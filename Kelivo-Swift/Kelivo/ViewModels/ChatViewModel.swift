...
final class ChatViewModel {
    ...
    // MARK: - Message Versioning

    /// Append a new version of a message (same groupId, incremented version).
    /// Mirrors Flutter's `ChatService.appendMessageVersion()`.
    func appendMessageVersion(
        conversationId: String,
        originalMessageId: String,
        newContent: String,
        role: String? = nil
    ) {
        guard let context = modelContext, let conversation = currentConversation,
              conversation.id == conversationId else { return }

        // Find the original message
        guard let original = messages.first(where: { $0.id == originalMessageId }) else { return }

        let groupId = original.groupId ?? original.id

        // Find current max version within this group
        let maxVersion = messages
        ...
        conversation.messageIds.append(newMessage.id)
    ...
    /// Set the selected version index for a message group.
    /// Mirrors Flutter's `ChatService.setSelectedVersion()`.
    func setSelectedVersion(conversationId: String, groupId: String, version: Int) {
    ...
    /// Get version selections map for a conversation.
    /// Mirrors Flutter's `ChatService.getVersionSelections()`.
    func getVersionSelections(conversationId: String) -> [String: Int] {
        guard let conversation = currentConversation,
              conversation.id == conversationId else { return [:] }

        return conversation.versionSelections
    }

    // MARK: - Message Truncation

    /// Toggle truncation at the tail of the current messages.
    /// When truncateIndex equals messageIds.count, it is active; toggle resets to -1.
    /// Mirrors Flutter's `ChatService.toggleTruncateAtTail()`.
    func toggleTruncateAtTail(conversationId: String) {
        guard let conversation = currentConversation,
              conversation.id == conversationId else { return }

        let lastIndexPlusOne = conversation.messageIds.count
        let newValue = (conversation.truncateIndex == lastIndexPlusOne) ? -1 : lastIndexPlusOne
        conversation.truncateIndex = newValue
        conversation.updatedAt = .now
    ...
    /// Update conversation summary generated by LLM.
    /// Mirrors Flutter's `ChatService.updateConversationSummary()`.
    func updateConversationSummary(conversationId: String, summary: String, messageCount: Int) {
        guard let conversation = currentConversation,
              conversation.id == conversationId else { return }
    ...
    /// Clear the summary of a conversation.
    /// Mirrors Flutter's `ChatService.clearConversationSummary()`.
    func clearConversationSummary(conversationId: String) {
        guard let conversation = currentConversation,
              conversation.id == conversationId else { return }
    ...
    private func buildMessagePayloads() -> [MessagePayload] {
        ...
        let truncateIndex = conversation.truncateIndex
        ...
        for message in messages {
            ...
            // Respect truncation: skip messages at or beyond truncateIndex
            if truncateIndex >= 0 && visibleIndex >= truncateIndex {
                break
            }
            visibleIndex += 1
